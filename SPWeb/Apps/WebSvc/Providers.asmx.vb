...
Namespace Apps.WebSvc
    ...
    ''' Class	 : SP.Web.Apps.WebSvc.Providers
    ''' 
    ''' -----------------------------------------------------------------------------
    ''' <summary>
    ''' Web service to allow interaction with the provider screens.
    ''' </summary>
    ''' <remarks>
    ''' </remarks>
    ''' <history><![CDATA[
    '''     MikeVO  29/09/2006  Changed FetchProviderList to return 10 records per page.
    ''' 	[paul]	28/09/2006	Created
    ''' ]]></history>
    ''' -----------------------------------------------------------------------------
    <System.Web.Services.WebService(Namespace:="http://targetsys.co.uk/TargetWeb/SPWeb/Apps/WebSvc/Providers")> _
...
#Region " FetchProviderList "
        ...
        ''' -----------------------------------------------------------------------------
        ''' <summary>
        '''     Retrieves a list of Providers for the specified user.
        ''' </summary>
        ''' <param name="page">The page in the resultset to view.</param>
        ''' <param name="selectedProviderID">The ID of the provider to select.</param>
        ...
        '''     MikeVO  09/10/2006  Added selectedProviderID param.
        '''     MikeVO  29/09/2006  Changed to return 10 records per page.
        ''' 	[PaulW]	19/09/2006	Created
        ''' ]]></history>
        ''' -----------------------------------------------------------------------------
        <WebMethod(EnableSession:=True), AjaxPro.AjaxMethod(AjaxPro.HttpSessionStateRequirement.Read)> _
        Public Function FetchProviderList(ByVal page As Integer, ByVal selectedProviderID As Integer, _
                                        ByVal listFilterReference As String, ByVal listFilterName As String) As FetchProviderListResult

            Dim msg As ErrorMessage
            Dim Providers As vwListProviderCollection = Nothing
            Dim result As FetchProviderListResult = New FetchProviderListResult
            ...
            Try
                ...
                msg = SPClassesBL.FetchProviders(conn, userID, page, pageSize, selectedProviderID, listFilterReference, listFilterName, totalRecords, Providers)
                If Not msg.Success Then
                    result.ErrMsg = msg
                    Return result
                End If
                ...
                With result
                    ...
                    .PagingLinks = Target.Library.Web.Utils.BuildPagingLinks( _
                        "<a href=""javascript:FetchProvList({0})"" title=""{2}"">{1}</a>&nbsp;", _
