Ext.define("TargetExt.ServiceUserImportException.MultiRecordEditor", {
    extend: 'Ext.window.Window',
    requires: [
        'Target.ux.form.field.ClearButton'
    ],
    closeAction: 'hide',
    title: 'Edit All Service User Exception(s)',
    resizable: false,
    width: 500,
    modal: true,
    config: {
    ...
    save: function () {
        var me = this, cfg = me.config;
        me.setLoading('Updating Service User Exception....');
        me.params.newStatusId = cfg.frmGeneralprocessStatusCollCmb.getValue();
        me.params.newSubStatusId= cfg.frmGeneralprocessSubStatusCollCmb.getValue();
        me.params.newAssignedToId= cfg.frmGeneralWebSecurityUser.getValue().ID;
    ...
    show: function (args) {
        ...
        me.resetFormVisibility();
        if (cfg.frmGeneralprocessStatusCollCmb.getValue() == cfg.frmGeneralprocessStatusColl.findRecord('Type', 4).data.ID) {
            cfg.frmGeneralprocessStatusCollCmb.setReadOnly(true);
        }
        else {
            cfg.frmGeneralprocessStatusColl.filter([{
                filterFn: function (item) {
                    return item.get("Type") < 4;
                }
            }]);
