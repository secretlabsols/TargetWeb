Ext.define('Results.Controls.ServiceUserResults', {
    extend: 'Results.ResultControl',
    requires: [
        'Target.ux.menu.ViewMenuItem',
        'Searchers.Controls.ServiceUserSearcher'
    ],
    initComponent: function () {
        var me = this, cfg = me.config;
        cfg.isFirstLoad = true;
        // add selector control
        cfg.selectorControl = new Selectors.ServiceUserSelector({
            request: {
                PageSize: 0
            },
            showFilters: false
        ...
        // add search panel
        cfg.searchPanel = Ext.create('Searchers.Controls.ServiceUserSearcher', { resultSettings: me.resultSettings });
        // call parents init
    ...
    actionOnSingleResultItem: function () {
        ...
        if (itemCount == 1 && cfg.isFirstLoad == false) {
            window.location.href = me.getUrlForServiceUser(cfg.selectorControl.GetItems()[0]);
        }
    },
    ...
    getViewMenuItems: function (item) {
        ...
        viewMenuItems.push({ href: me.getUrlForServiceUser(item), key: 'ServiceUser', text: 'View', clickMessage: 'Opening Service User...', tooltip: 'View Service User?' });
        return viewMenuItems;
    },
    getUrlForServiceUser: function (item) {
        var me = this, cfg = me.config, url = '#';
        if (item) {
            return cfg.svcUserUrl + '?clientid=' + item.ID.toString() + '&mode=1&visited=true' + '&backUrl=' + escape(me.getBackUrl());
        }
        return url;
    },
