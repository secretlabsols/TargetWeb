...
Namespace Apps.Reports.Charting
    ...
    ''' </history>
    Public Class BySvcTypeBudgetCategory
        ...
        Protected Overrides Sub PopulateChartData(ByRef seriesList As System.Web.UI.DataVisualization.Charting.SeriesCollection)
            ...
            Dim dateTo As Date
            Dim svcClassificationID As Integer
            ...
            If Not Me.queryString("svcClassificationID") Is Nothing And Not Me.queryString("svcClassificationID") = "" Then
                svcClassificationID = Me.queryString("svcClassificationID")
            End If

            msg = FetchReportData(Me.DbConnection, svcClassificationID, dateFrom, dateTo, dtData)
            If msg.Success Then

                'populate outer section (Service Type)
                Dim svcType As String = String.Empty
                Dim totalValue As Double = 0
                For Each row As DataRow In dtData.Rows
                    If svcType <> String.Empty And svcType <> row("Service Type") Then
                        outer.Points.AddXY(svcType, totalValue)
                        totalCommitment += totalValue
                        svcType = row("Service Type")
                ...
                For Each Point As DataPoint In outer.Points
                    ...
                    Point.Url = String.Format("javascript:CommSvcTypeBudgCat_ServiceType('{0}');", "#VALX")
                ...
                For Each row As DataRow In dtData.Rows
                    inner.Points.AddXY(row("Budget Category"), row("Total Value"))
                Next
                ...
                For Each Point As DataPoint In inner.Points
                    ...
                    Point.ToolTip = _
                        String.Format("{0} represents {1} of the commitment for the period {2} - {3}, totalling {4}", _
                                                            ...
                                                            DateFrom.ToShortDateString, dateTo.ToShortDateString, "#VALY{C}")
                    Point.LegendToolTip = Point.ToolTip
                    Point.Url = String.Format("javascript:CommSvcTypeBudgCat_BudgetCategory('{0}');", "#VALX")
                    Point.LegendUrl = Point.Url
                    'Alter the height of the legen depending on the data
                    Me.Chart.Legends("Default").Position.Height = Me.Chart.Legends("Default").Position.Height + 3
                Next

                'Populate the center section of the pie chart
                ...
                center.Points(0).Url = "javascript:CommSvcTypeBudgCat_All();"
        ...
        Private Shared Function FetchReportData(ByVal conn As SqlConnection, _
                                                ByVal svcClassificationID As Integer, _
                                                ByVal dateFrom As Date, _
                                                ByVal dateTo As Date, _
                                                ByRef dtData As DataTable) As ErrorMessage

            Const SP_NAME As String = "spxChart_CommitmentBySvcTypeBudgetCategory"
            ...
            Try
                ...
                If svcClassificationID Then spParams(0).Value = svcClassificationID
                If IsDate(dateFrom) Then spParams(1).Value = dateFrom
                If IsDate(dateTo) Then spParams(2).Value = dateTo
