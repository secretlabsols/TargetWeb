...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <!--
		Abacus for Windows Job Service - Debtors Interface - Create XML Input.
		
		Formats the Exports.FinancialExportInterface.Debtors.CreateXML input information for display.
	-->
    <xsl:template match="InterfaceLogID">
        Debtor Invoice Batch ID: <xsl:value-of select="." /><br />
	</xsl:template>

    <xsl:template match="InterfaceLogRef">
        Debtor Invoice Batch Reference: <xsl:value-of select="." /><br />
    </xsl:template>
