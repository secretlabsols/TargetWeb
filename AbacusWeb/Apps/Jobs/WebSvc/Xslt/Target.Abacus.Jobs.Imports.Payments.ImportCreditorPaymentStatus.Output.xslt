...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<!--
		Abacus for Windows Job Service - Dom Service Order Suspensions Import - Load Interface File Output.
		
		Formats the Imports.DomServiceOrderSuspensions.LoadInterfaceFile output information for display.
	-->
	<xsl:template match="Results">
		<xsl:value-of select="Count" /> payment status records were processed from
		<xsl:choose>
			...
			<xsl:otherwise>
				...
				<xsl:if test="number(ExceptionsCount) &gt; 0">
					...
					<b>IMPORTANT</b>
					<br />
					<xsl:value-of select="ExceptionsCount" /> exception(s) were raised during processing. These payment statuses were not processed.
					<br />
