...
Namespace Apps.InPlaceSelectors
    ...
    ''' </history>
    Partial Public Class InPlaceServiceSelector
...
#Region "Fields"
        ...
        ' constants
        Private Const _InPlaceSelectorJavascriptLibraryKey As String = "InPlaceServiceSelectorJavascriptLibrary"
        Private Const _InPlaceSelectorJavaScriptPath As String = "AbacusWeb/Apps/InPlaceSelectors/InPlaceServiceSelector.js"
        Private Const _InPlaceSelectorName As String = "InPlaceServiceSelector"
...
#Region "Methods"
        ...
        Private Sub LoadItem()
            ...
            Dim item As ViewableService = New ViewableService()
            ...
            If ItemID > 0 Then
                ...
                msg = AbacusClassesBL.FetchServiceWithDetails(BasePage.DbConnection, ItemID, item)
                If Not msg.Success Then WebUtils.DisplayError(msg)

                ' set the controls on the selector to that of the fetched item
                txtName.Value = item.Title
                hidID.Value = item.ID
            ...
            js.AppendFormat("{0} = new {1}_Service();", jsServiceObjectVar, _InPlaceSelectorName)
            js.AppendFormat("{0}.ID = {1};", jsServiceObjectVar, item.ID)
            js.AppendFormat("{0}.BudgetCategoryDescription = '{1}';", jsServiceObjectVar, item.BudgetCategoryDescription)
            js.AppendFormat("{0}.BudgetCategoryID = {1};", jsServiceObjectVar, item.BudgetCategoryID)
            js.AppendFormat("{0}.Description = '{1}';", jsServiceObjectVar, item.Description)
            js.AppendFormat("{0}.DontTreatAsMinutes = {1};", jsServiceObjectVar, item.DontTreatAsMinutes.ToString().ToLower())
            js.AppendFormat("{0}.EnableVisits = {1};", jsServiceObjectVar, item.EnableVisits.ToString().ToLower())
            js.AppendFormat("{0}.ForUseWithDomiciliaryContracts = {1};", jsServiceObjectVar, item.ForUseWithDomiciliaryContracts.ToString().ToLower())
            js.AppendFormat("{0}.MeasuredIn = '{1}';", jsServiceObjectVar, item.MeasuredIn)
            js.AppendFormat("{0}.MinuteConversion = {1};", jsServiceObjectVar, item.MinuteConversion)
            js.AppendFormat("{0}.MinuteRounding = {1};", jsServiceObjectVar, item.MinuteRounding)
            js.AppendFormat("{0}.Title = '{1}';", jsServiceObjectVar, item.Title)
            js.AppendFormat("{0}.VisitBasedReturns = {1};", jsServiceObjectVar, item.VisitBasedReturns.ToString().ToLower())
            js.AppendFormat("{0}.IsInUseByRateCategory = {1};", jsServiceObjectVar, item.IsInUseByRateCategory.ToString().ToLower())
            js.AppendFormat("{0}.IsInUseBySdsInvoicedDomServiceActualWeek = {1};", jsServiceObjectVar, item.IsInUseBySdsInvoicedDomServiceActualWeek.ToString().ToLower())
