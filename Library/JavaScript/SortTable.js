...
function ts_makeSortable(table) {
    ...
    for (var i=0;i<firstRow.cells.length;i++) {
        ...
        cell.innerHTML = '<a href="#" title="Sort by this column" onclick="ts_resortTable(this);return false;">'+txt+'<img class="sortarrow" src="'+SITE_VIRTUAL_ROOT+'Images/clear.gif" alt="Sort Order" /></a>';
    }
}
...
function ts_resortTable(lnk) {
    ...
    if (img.getAttribute("sortdir") == 'down') {
        ARROW = SITE_VIRTUAL_ROOT + 'Images/downsimple.png';
        newRows.reverse();
        img.setAttribute('sortdir','up');
        img.alt = "Ascending"
    } else {
        ARROW = SITE_VIRTUAL_ROOT + 'Images/upsimple.png';
    ...
    for (var ci=0;ci<allimgs.length;ci++) {
        if (allimgs[ci].className == 'sortarrow') {
            if (getParent(allimgs[ci],"table") == getParent(lnk,"table")) { // in the same table as us?
                allimgs[ci].src = SITE_VIRTUAL_ROOT + 'Images/clear.gif';
            }
        }
    }
