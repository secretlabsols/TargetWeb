 ...
 * @author Mike van Oorschot - changed strftime() to return empty string if date is min date.
 * @version 0.2
 */
 ...
 * ========================
 */
Date.strftime = function(format, dt, filterEmptyNetDate) {
    var dtParam = null;
    var patArr = [];
    var dtArr = [];
    var str = format;
    ...
    // MvO 04/08/2009 - if min date then return empty string
    if(filterEmptyNetDate == undefined) filterEmptyNetDate = true;
    if(filterEmptyNetDate) {
        var emptyDate = new Date(1901, 0, 1, 0, 0, 0);
        if(dtParam.valueOf() == emptyDate.valueOf()) return " ";
    }
...
Date.diff = function(interv, dt1, dt2) {
    ...
    */
    ...
    var dtParam1 = (typeof dt1 == 'number') ? new Date(dt1) : dt1; // Convert Unix timestamp to Date obj if need be
    var dtParam2 = (typeof dt2 == 'number') ? new Date(dt2) : dt2; // Convert Unix timestamp to Date obj if need be
    var intervFlag = interv.toLowerCase();
    // Year, month
    ...
    var weeDiff = dayDiff/7; // Counts number of seven-day intervals, not calendar weeks
    var ret = 0;
    ...
    switch(intervFlag) {
        case 'yyyy':
            ...
            break;
        case 'q':
            alert('Format not yet supported.');
            return false;
            break;
        case 'm':
            ret = monDiff;
...
//===========================================================================
// Description: Formats JSON dates to one javascript can understand
// Source: http://stackoverflow.com/questions/1224793/javascript-serialization-of-datetime-in-asp-net-is-not-giving-a-javascript-date-o
// Created 09/09/2011 Motahir D11766 - e-Invoicing Provider Invoice Tolerances
//=========================================================================== 
Date.getFormatedJSONDate = function(value) {
    return value = new Date(parseInt(value.replace("/Date(", "").replace(")/", ""), 10));
}
//===========================================================================
// Description: Determine if a string contains a valid date expression
// Source: http://essentialgeek.blogspot.com/2006/11/creating-isdate-function-in-javascript.html
// Created 09/09/2011 Motahir D11766 - e-Invoicing Provider Invoice Tolerances
//=========================================================================== 
Date.isDate = function(sDate) {
...
Date.advanceDateToDay = function(goToDayDate, dateToAdvance) {
    if (goToDayDate && dateToAdvance) {
        var fromDay = dateToAdvance.getDay(), toDay = goToDayDate.getDay();
        while (fromDay != toDay) {
            dateToAdvance = Date.add('d', 1, dateToAdvance);
            fromDay = dateToAdvance.getDay();
        }
        return dateToAdvance;
    } else {
        return null;
    }
}

Date.toDateFromString = function(value) {
    return new Date(value.split('/')[2], value.split('/')[1] - 1, value.split('/')[0]);
}

Date.convertDateToUTC = function (value) {
    return new Date(value);
}

Date.isoFormat = "([0-9]{4})(-([0-9]{2})(-([0-9]{2})" +
                    "(T([0-9]{2}):([0-9]{2})(:([0-9]{2})(\.([0-9]+))?)?" +
                    "(Z|(([-+])([0-9]{2}):([0-9]{2})))?)?)?)?";

Date.isIsoDate = function (value) {
    var matches = [];
    if (!value) {
        return matches;
    }
    matches = (value.match(new RegExp(Date.isoFormat)) || []);
    return (matches.length > 0);
}

Date.fromIsoDate = function (value) {
    var dateToReturn = new Date(), offset = 0, time = 0;
    if (value && Date.isIsoDate(value)) {
        var matches = [];
        if (!value) {
            return matches;
        }
        matches = (value.match(new RegExp(Date.isoFormat)) || []);
        dateToReturn = new Date(matches[1], 0, 1);
        if (matches[3]) { dateToReturn.setMonth(matches[3] - 1); }
        if (matches[5]) { dateToReturn.setDate(matches[5]); }
        if (matches[7]) { dateToReturn.setHours(matches[7]); }
        if (matches[8]) { dateToReturn.setMinutes(matches[8]); }
        if (matches[10]) { dateToReturn.setSeconds(matches[10]); }
        if (matches[12]) { dateToReturn.setMilliseconds(Number("0." + matches[12]) * 1000); }
        if (matches[14]) {
            offset = (Number(matches[16]) * 60) + Number(matches[17]);
            offset *= ((matches[15] == '-') ? 1 : -1);
        }
        offset -= dateToReturn.getTimezoneOffset();
        time = (Number(dateToReturn) + (offset * 60 * 1000));
