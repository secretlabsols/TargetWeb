...
function dteDateFrom_Changed(id) {
    ...
    var $dateToCtrl = $('#' + SelectorWizard_dateToID + '_txtTextBox');
    var maxdate = $dateFromCtrl.val().toDate()
    maxdate = Date.add('m', 1, maxdate);
    $dateToCtrl.datepicker('option', 'minDate', $dateFromCtrl.val().toDate());
    $dateToCtrl.datepicker('option', 'maxDate', maxdate);
